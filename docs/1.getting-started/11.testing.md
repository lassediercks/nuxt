...
> **Note**: If your component sends requests to an external API, you can configure a baseURL for [fetch](/docs/api/utils/dollarfetch) operations based on environment variables. By setting this baseURL to an empty string through [Nuxt's Environment Configuration](/docs/guide/directory-structure/env#custom-file) (using .env.test), you ensure that all requests are directed to the Nitro server.
